...
void Renderer::Initialize()
{
	...
	sceneColorDescriptorHandle_ = TextureManager::GetInstance()->GetDescriptorHeap()->Allocate();
	device->CopyDescriptorsSimple(1, sceneColorDescriptorHandle_, sceneColorBuffer_->GetSRVHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

	//深度バッファの作成
	sceneDepthBuffer_ = std::make_unique<DepthBuffer>();
	sceneDepthBuffer_->Create(Application::kClientWidth, Application::kClientHeight, DXGI_FORMAT_D32_FLOAT, true);

	//ディスクリプターをコピー
	sceneDepthDescriptorHandle_ = TextureManager::GetInstance()->GetDescriptorHeap()->Allocate();
...
void Renderer::PreDraw()
{
	...
	commandContext->SetRenderTargets(1, &sceneColorBuffer_->GetRTVHandle(), sceneDepthBuffer_->GetDSVHandle());
...
void Renderer::CreateModelPipelineState()
{
	...
	for (uint32_t i = 0; i < 2; i++) {
		...
		newPipelineState.SetRenderTargetFormats(1, &rtvFormat, DXGI_FORMAT_D32_FLOAT);
...
void Renderer::CreateSpritePipelineState()
{
	...
	//Shaderをコンパイルする
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob = ShaderCompiler::CompileShader(L"SpriteVS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = ShaderCompiler::CompileShader(L"SpritePS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
