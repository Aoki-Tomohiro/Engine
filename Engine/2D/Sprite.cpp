...
void Sprite::Draw()
{
	...
	commandContext->SetConstantBuffer(0, materialResource_->GetGpuVirtualAddress());
	...
	commandContext->SetDescriptorTable(2, textureManager->GetDescriptorHandle(textureName_));
...
void Sprite::CreateMaterialResource()
{
	...
	materialResource_->Create(sizeof(ConstBuffDataMaterial));
	//リソースに書き込む
	UpdateMaterialResource();
}
...
{
	//書き込むためのアドレスを取得
	ConstBuffDataMaterial* materialData = static_cast<ConstBuffDataMaterial*>(materialResource_->Map());
	//データを書き込む
