...
{
	//ジャンプ攻撃アニメーションを再生する
	enemy_->GetAnimator()->PlayAnimation("JumpAttack", 1.0f, false);
	...
	//移動終了座標を設定
	endPosition_ = GameObjectManager::GetInstance()->GetGameObject<Player>("Player")->GetJointWorldPosition("mixamorig:Hips");
...
void EnemyStateJumpAttack::Update()
{
	...
	if (animator->GetIsAnimationFinished())
	{
		...
		enemy_->SetPosition(enemy_->GetPosition() + Vector3(enemy_->GetJointLocalPosition("mixamorig:Hips").x, 0.0f, enemy_->GetJointLocalPosition("mixamorig:Hips").z));
