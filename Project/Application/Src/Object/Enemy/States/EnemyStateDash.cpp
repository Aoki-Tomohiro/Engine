...
{
	//ダッシュアニメーションを再生
	enemy_->PlayAnimation("Dash", 1.0f, false);
...
void EnemyStateDash::Update()
{
	...
	if (isDashFinished_ && enemy_->GetIsBlendingCompleted())
	{
...
{
	//アニメーションの進行時間を取得
	return enemy_->GetIsBlendingCompleted() ? enemy_->GetCurrentAnimationTime() : enemy_->GetNextAnimationTime();
}

const float EnemyStateDash::GetAnimationDuration() const
{
	//アニメーションの総時間を取得
	return enemy_->GetIsBlendingCompleted() ? enemy_->GetCurrentAnimationDuration() : enemy_->GetNextAnimationDuration();
}